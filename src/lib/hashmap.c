...
#define HASHFUN hash_bkdr
unsigned int ( *hash_callback )( char *keystring );

/* Structure for holding pair Key & Value */
...
/* Hashmap has some maximum size and current size, as well as the data to hold. */
typedef struct _map {
        size_t table_size;
        size_t size;
        map_element *data;
...
static const unsigned long num_primes             = 28;
...
map_t map_create( size_t size ) {
        ...
        new->data = calloc( size, sizeof( map_element ) );
...
bool map_resize( map_t *in, size_t room ) {
        ...
        size_t size = next_prime( p_map->table_size + room );

        map_element *data = realloc( p_map->data, size * sizeof( map_element ) );
 ...
 * I have borrowed the tables directly, and made some minor changes to the
...
static unsigned long crc32( const unsigned char *s, size_t len ) {
        ...
        for ( size_t i = 0; i < len; i++ ) {
                crc32_val = crc32_tab[( crc32_val ^ s[i] ) & 0xFF] ^ ( crc32_val >> 8 );
        }
        return crc32_val;
}

static unsigned int map_hash_int_diff( map *m, char *keystring ) {
        unsigned int key = hash_callback( keystring );
        return key % m->table_size;
}

static unsigned int map_hash_int( map *m, char *keystring ) {
...
/**
  ...
  */
static int map_hash( map_t in, char *key ) {
        int curr;
        ...
        if ( m->size >= ( m->table_size / 2 ) )
                return ARDP_MAP_FULL;

        /* Find best index */
        curr = map_hash_int_diff( in, key );

        /* Linear probing */
        for ( size_t i = 0; i < MAX_CHAIN_LENGTH; i++ ) {
                if ( m->data[curr].in_use is false )
                        return curr;
                else if ( strcmp( m->data[curr].key, key ) is 0 )
                        return curr;
                else
...
/**
  ...
  */
static int map_rehash( map_t in ) {
        int old_size;
        map_element *curr;
        ...
        map_element *temp = ( map_element * )calloc( nextSize, sizeof( map_element ) );
        if ( not temp )
                return ARDP_MAP_OUTMEM;
        ...
        for ( size_t i = 0; i < old_size; i++ ) {
                ...
                status = map_push( m, curr[i].key, curr[i].data );
...
int map_push( map_t in, char *key, var value ) {
        ...
        index = map_hash( in, key );
        while ( index is ARDP_MAP_FULL ) {
                if ( map_rehash( in ) is ARDP_MAP_OUTMEM ) {
                        return ARDP_MAP_OUTMEM;
                }
                index = map_hash( in, key );
        }
         ...
         */
        if ( m->data[index].in_use is true ) {
                return ARDP_MAP_USED;
        }
        /* Set the data */
...
int map_get( map_t in, char *key, var *arg ) {
        ...
        for ( size_t i = 0; i < MAX_CHAIN_LENGTH; i++ ) {
                ...
                if ( in_use is true ) {
                        if ( strcmp( m->data[curr].key, key ) is 0 ) {
 ...
 * The additional var argument is passed to the function as its first
...
int map_iterate( map_t in, map_callback f, var item ) {
        ...
        for ( i = 0; i < m->table_size; i++ )
                if ( m->data[i].in_use isnt false ) {
                        var data   = ( var )( m->data[i].data );
                        int status = f( item, data );
                        if ( status isnt ARDP_MAP_OK )
                                return status;
                }
